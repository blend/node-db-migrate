...
exports.connect = function(config, callback) {
  // throw new Error(`TEST ERROR ${JSON.stringify(config, '', 2)}`);
  var port;
  var host;
  var replSet;
  ...
  var servers = hosts.map(function(host) {
    // throw new Error(`TEST ERROR 0 ${mongoString}, ${options}`);
  ...
  if(servers.length > 2) {
    ...
    // throw new Error(`TEST ERROR 1 ${mongoString}`);
    mongoClient = new MongoClient(new ReplSet(servers, options));
  } else {
    // throw new Error(`TEST ERROR 2 ${mongoString}`);
